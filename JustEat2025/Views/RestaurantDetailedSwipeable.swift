//
//  RestaurantDetailedSwipeable.swift
//  JustEat2025
//
...
struct SwipeableCardsView: View {
    class Model: ObservableObject {
        private let originalCards: [RestaurantDetailed.Model]
        @Published var unswipedCards: [RestaurantDetailed.Model]
        @Published var swipedCards: [RestaurantDetailed.Model]
        
        init(cards: [RestaurantDetailed.Model]) {
        ...
        func updateTopCardSwipeDirection(_ direction: RestaurantDetailed.SwipeDirection) {
            if !unswipedCards.isEmpty {
            ...
            if model.unswipedCards.isEmpty && model.swipedCards.isEmpty {
                ...
                ZStack {
                    ...
                    ForEach(model.unswipedCards.reversed()) { card in
                        let isTop = card.id == model.unswipedCards.first?.id
                        let isSecond = card.id == model.unswipedCards.dropFirst().first?.id
                            ...
                            DragGesture()
                                ...
                                .onEnded { _ in
                                    if abs(self.dragState.width) > swipeThreshold {
                                        let swipeDirection: RestaurantDetailed.SwipeDirection = self.dragState.width > 0 ? .right : .left
